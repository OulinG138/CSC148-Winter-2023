...
class UserPlayer(Player):
    ...
    def move(self, current: int, min_step: int, max_step: int, goal: int) -> int:
        move = input(f'Enter the current step: {current}, min step: {min_step}, \
                      max step: {max_step}, and the end goal: {goal}.')
        if goal > current + steps:
...
def make_player(generic_name: str) -> Player:
    ...
    type_ = input(f'Enter a player type for {generic_name} (U, R, S)')
    while type_ not in ['U' , 'R', 'S']:
        type_ = input('Try Again (U, R, S)')
