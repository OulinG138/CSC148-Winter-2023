...
################################################################################
def test_add_in_order_different_items() -> None:
    """Test the add_in_order function with different items in an empty list"""
    stack = Stack()
    lst = [1, 2, 3]
...
def test_peek_with_empty_stack() -> None:
    """Test that peek works with an empty stack."""
    stack = Stack()
...
@given(lists(integers(), min_size=2, max_size=100))
def test_reverse_top_two_general(items: list[int]) -> None:
    """Test that reverse_top_two works for a large range of stack 
    sizes."""
    stack = Stack()
    for item in items:
        stack.push(item)
    reverse_top_two(stack)
    assert stack.pop() == items[-2]
    assert stack.pop() == items[-1]
...
def test_remove_all_general(items: list[int]) -> None:
    """Test that remove_all works for a large range of queue 
    sizes."""
    queue = Queue()
    for item in items:
        queue.enqueue(item)
...
@given(lists(integers(), min_size=1, max_size=100))
def test_remove_all_general(items: list[int]) -> None:
    """Test that remove_all_but_one works for a large range of queue 
    sizes."""
    queue = Queue()
    for item in items:
        queue.enqueue(item)
