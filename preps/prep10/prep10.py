...
class Tree:
    ...
    def swap_down(self) -> None:
        ...
        if self.is_empty() or self._subtrees == []:
            ...
            tree_to_swap = max(self._subtrees, key=lambda tree: tree._root)
            if self._root < tree_to_swap._root:
                self._root, tree_to_swap._root = tree_to_swap._root, self._root
                tree_to_swap.swap_down()
