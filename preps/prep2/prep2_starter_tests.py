...
@given(size=integers(min_value=1),
       force=lists(integers(min_value=0), min_size=3, max_size=3))
def test_spin(size: int, force: list[int]) -> None:
    """Test that the position of the spinner is always smaller or equal to 
    size - 1."""
    spinner = Spinner(size)
    a, b, c = force[0], force[1], force[2]

    spinner.spin(a)
    assert spinner.position <= size - 1
...
def test_spin_randomly_equal_chance() -> None:
    """Test a spinner with three slots and whether each slot has an
    approximately equal chance of being pointed to."""
    spinner = Spinner(3)
    dic = {}

    for _ in range(10000):
        spinner.spin_randomly()
        if spinner.position not in dic:
            dic[spinner.position] = 1
        else:
            dic[spinner.position] += 1

    test_range = lambda x: 3000 <= x <= 3500

    assert all([test_range(num) for num in list(dic.values())])
...
def test_unlike_with_random_likes(n: int) -> None:
    """Test unlike method with a random number of likes."""
    tweet = Tweet('Sophia', date(2021, 1, 1), 'Happy new year!')
    tweet.like(n)
    assert tweet.likes == n
    tweet.unlike()
    assert tweet.likes == n - 1
