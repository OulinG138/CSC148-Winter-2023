...
import random # used to generate random numbers from typing import Dict, Optional, Tuple
...
class Vehicle:
	...
	def __init__(self, initial_fuel: int, initial_position: Tuple[int, int]) -> \
	None: 
	...
	def move(self, new_x: int, new_y: int) -> None: 
		...
		if needed <= self.fuel:
			self.position = (new_x, new_y) self.fuel -= needed
...
class Car(Vehicle):
	...
	def __init__(self, initial_fuel: int) -> None:
		# Make sure position is (0, 0)
		Vehicle.__init__(self, initial_fuel, (0, 0))	

	def fuel_needed(self, new_x: int, new_y: int) -> int:
		# The amount of fuel needed is change in x and y
		change_in_x = abs(self.position[0] - new_x)
		change_in_y = abs(self.position[1] - new_y)
...
class UnreliableMagicCarpet(Vehicle):
	...
	def __init__(self, initial_fuel: int) -> None:
		starting_porint = (randint(-10, 10), random.randint(-10, 10))	
		Vehicle.__init__(self, initial_fuel, starting_porint)
...
class SuperDuperManager:
	...
	def move_vehicle(self, id_: str, new_x: int, new_y: int) -> None:
		...
		The vehicle called <id_> should be moved to position (<new_x>, <new_y>). Do nothing if there is no vehicle with the given id, or if the corresponding vehicle does not have enough fuel to move.
		"""
	...
	def get_vehicle_position(self, id_: str) -> Optional[Tuple[int, int]]:
		...
		Return a tuple of the (x, y) position of the vehicle. Return None if 
		there is no vehicle with the given id. 
		"""
		if id_ in self._vehicles:
